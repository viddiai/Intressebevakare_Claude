  ...
  const configs: ImapConfig[] = [
    {
      ...
      host,
      port: 993,
      user: (process.env.IMAP_TROLLHATTAN_USER || "").trim(),
      password: (process.env.IMAP_TROLLHATTAN_PASSWORD || "").trim(),
    },
    {
      name: "Göteborg",
      anlaggning: "Göteborg",
      host,
      port: 993,
      user: (process.env.IMAP_GOTEBORG_USER || "").trim(),
      password: (process.env.IMAP_GOTEBORG_PASSWORD || "").trim(),
    },
    {
      name: "Falkenberg",
      ...
      host,
      port: 993,
      user: (process.env.IMAP_FALKENBERG_USER || "").trim(),
      password: (process.env.IMAP_FALKENBERG_PASSWORD || "").trim(),
    },
  ];
